...
describe DonationHelper do
  ...
  describe '#donation_hidden_tag' do
    ...
    it { expect(helper.donation_hidden_tag(model, 'value')).to eq tag :input, value: model.value, type: 'hidden', name: "maintainer[donations_attributes][#{model.id}][value]", id: "maintainer_donations_attributes_#{model.id}_value" }
  end
